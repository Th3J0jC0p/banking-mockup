            ...
            {transactions.map((t: Transaction) => {
                ...
                return (
                    <TableRow key={t.id} className={`${isDebit || amount[0] === '-' ? 'bg-[#fffbfa]' : 'bg-[#f6fef9]'} !over:bg-none !border-b-DEFAULT `}>
                        <TableCell className="max-w-[250px] pl-2 pr-5 max-md:max-w-[150px]">
                            <div className='flex items-center gap-3'>
                                <h1 className='text-14 truncate font-semibold text-[#344054]'>
                                    {removeSpecialCharacters(t.name) || 'No description'}
                                </h1>
                            </div>
                        </TableCell>
                        <TableCell className={`${isDebit || amount[0] === '-' ? 'text-[#f04438]' : 'text-[#039855]'} pl-2 pr-5 font-semibold `}>{isDebit ? `-${amount}` : isCredit ? amount : amount}</TableCell>
                        <TableCell className="pl-2 pr-5"><CategoryBadge category={status}/></TableCell>
                        <TableCell className="min-w-24 pl-2 pr-10">{formatDateTime(new Date(t.date)).dateTime}</TableCell>
                        <TableCell className="pl-2 pr-5 capitalize min-w-24 max-md:hidden">{t.paymentChannel}</TableCell>
